...
# ---------- worker (inner unit) ----------
def run_distil_ewc_grid(scenario: str, metadata: str, cfg: tuple[float,float,float, str]):
    """
...
def grid_search(
    ...
    with ProcessPoolExecutor(max_workers=inner_max_workers, mp_context=mp_ctx) as ex:
        futs = [ex.submit(run_distil_ewc_grid, scenario, metadata, cfg) for cfg in param_grid]
