...
def plot_overlay_accuracies_per_scenario(
    ...
    highlight_map: {method_name: True (vivid) / False (faded alpha=0.2)}
    """
    ...
    # -----------------------
    all_clients_vals = []  # collect all client accuracies for min/max
    ...
    # --- Compute and apply min/max for client plot ---
    if all_clients_vals:
        min_client = min(all_clients_vals)-0.05
    ...
    ax1.legend(
        loc="lower right", ncol=2, frameon=True, facecolor="white", framealpha=0.9,
    ...
    # -----------------------
    all_global_vals = []  # collect all global accuracies for min/max
    for idx, (agg_name, pairs) in enumerate(sorted(results_by_method.items())):
    ...
    # --- Compute and apply min/max for global plot ---
    if all_global_vals:
        min_global = min(all_global_vals)-.02
    ...
    # ✅ Return computed ranges for later inspection or tweaking
    return {
...
for subfolder in subfolders:
    ...
    for item in json_summary:
        ...
        highlight_map[agg_method]= random.random() < 0.2
